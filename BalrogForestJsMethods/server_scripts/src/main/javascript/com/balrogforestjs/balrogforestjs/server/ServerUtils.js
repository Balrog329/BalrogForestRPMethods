...
global.getServerContext = function() {
    ...
    const serverDir = `${serversPath}/${serverData.serverDir ?? serverName}`
...
function newProprety(name, owner, referent_manager) {
    ...
    global.proprieties[parseMcWorld(name)] = {
        name_id: name,
        id: Math.max.apply(null, Object.values(global.proprieties).map(f => Number(f.id))) + 1,
...
function buyProperty(property, buyer, referent_manager, amount) {
    ...
    let forest = global.proprieties[world]
